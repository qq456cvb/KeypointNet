...
class RSNet(nn.Module):
    def __init__(self, emb_dim, res=0.01, rg=1.5):
        super(RSNet, self).__init__()

        resolution_true = [res, res, res]

        # - modified resolution for easy indexing
        self.resolution = [i + 0.00001 for i in resolution_true]
        num_slice = [0, 0, 0]
        num_slice[0] = int(rg / self.resolution[0]) + 1
        num_slice[1] = int(rg / self.resolution[1]) + 1
        num_slice[2] = int(rg / self.resolution[2]) + 1
        ...
        self.conv_8 = nn.Conv2d(256, emb_dim, kernel_size=(1, 1), stride=(1, 1))
